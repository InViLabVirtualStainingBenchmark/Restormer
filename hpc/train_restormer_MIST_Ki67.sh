#!/bin/bash
#SBATCH --job-name=restormer_MIST_Ki67
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH -o /data/antwerpen/212/vsc21216/projects/logs/restormer_MIST_Ki67_%j.out
#SBATCH -e /data/antwerpen/212/vsc21216/projects/logs/restormer_MIST_Ki67_%j.err
set -euo pipefail
...
OUTPUT_DIR="$VSC_DATA/projects/outputs/restormer_MIST_Ki67_vaughan"
mkdir -p "$OUTPUT_DIR"
...
srun apptainer exec     --nv     -B "$CODE_DIR":/code     -B "$DATA_SQSH":/data:image-src=/     -B "$OUTPUT_DIR":/output     "$CONTAINER"     bash -c "
    ...
    bash /code/Restormer/train_restormer.sh     /code/Restormer/experiments/MIST_Ki67_Restormer/train_MIST_Ki67.yml
    "
